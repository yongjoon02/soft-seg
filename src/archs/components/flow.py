...
class SchrodingerBridgeConditionalFlowMatcher(ConditionalFlowMatcher):
    """Schrodinger Bridge conditional flow matching.
    
    This class implements Schrodinger Bridge flow matching where noise is only
    injected in the middle of the path. The noise is maximum at t=0.5 and zero
    at t=0 and t=1.
    ...
    def compute_mu_t(self, x0, x1, t):
        """Compute the mean of the probability path: linear interpolation.
        ...
        Returns
        -------
        mean mu_t: (1 - t) * x0 + t * x1
        """
        t = pad_t_like_x(t, x0)
        return (1 - t) * x0 + t * x1

    def compute_sigma_t(self, t):
        """Compute the standard deviation: maximum noise at t=0.5.
        
        The noise is zero at t=0 and t=1, and maximum at t=0.5.
        ...
        Returns
        -------
        standard deviation: sqrt(t * (1 - t)) * sigma
        """
        # Ensure t is a tensor for computation
        if isinstance(t, (float, int)):
            t = torch.tensor(t, dtype=torch.float32)
        
        # Clamp t to [0, 1] to avoid numerical issues
        t_clamped = torch.clamp(t, 0.0, 1.0)
        sigma_t = torch.sqrt(t_clamped * (1 - t_clamped) + 1e-8) * self.sigma
        return sigma_t
