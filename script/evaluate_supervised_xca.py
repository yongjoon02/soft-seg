"""
Evaluate all trained supervised models on XCA test data.

Usage Examples:
    # Basic usage (evaluate all supervised models on XCA)
    uv run python script/evaluate_supervised_xca.py
    
    # Specific models only
    uv run python script/evaluate_supervised_xca.py --models "cenet,csnet,aacaunet"
    
    # Custom output directory
    uv run python script/evaluate_supervised_xca.py --output_dir results/xca_supervised
    
    # Help
    uv run python script/evaluate_supervised_xca.py --help

Parameters:
    --models: Comma-separated model names (default: all supervised models)
...
def find_checkpoint(model_name):
    """Find checkpoint for supervised model on XCA dataset."""
...
def evaluate_model(model_name):
    """Evaluate single supervised model on XCA dataset."""
...
def main(models, output_dir):
    """Evaluate all supervised models on XCA dataset and create results table."""
    model_list = [m.strip() for m in models.split(',')]
    results = []
    
    print("="*80)
    print("EVALUATING SUPERVISED MODELS ON XCA DATASET")
    print("="*80)
    ...
    if results:
        ...
        print("\n" + "="*80)
        print("XCA SUPERVISED MODEL EVALUATION RESULTS")
        print("="*80)
        ...
        print("Make sure models are trained first using train_supervised_xca.sh")
        print("="*80)
