#!/bin/bash
# Train diffusion models on XCA dataset
...
for i in "${!MODELS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${gpu} python script/train_diffusion_models.py fit \
        --config ${CONFIG} \
        --arch_name ${model} \
        > logs/train_xca_diffusion_${model}.log 2>&1 &
...
echo ""
echo "=================================================="
echo "All diffusion models started. Check logs/ directory for outputs."
echo "To monitor: tail -f logs/train_xca_diffusion_*.log"
echo "=================================================="
